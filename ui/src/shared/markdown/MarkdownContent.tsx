...
export function MarkdownContent({ content, className = "" }: { content: string; className?: string }) {
  return (
    <article className={`markdown-content text-[14px] leading-[1.6] text-ink ${className}`}>
      <ReactMarkdown
        ...
        components={{
          ...
          code: ({ className: codeClassName, ...props }) => <code className={`rounded bg-page px-1 py-0.5 font-mono text-[13px] ${codeClassName ?? ""}`} {...props} />,
          pre: ({ ...props }) => <pre className="my-4 overflow-auto rounded-md border border-hairline bg-panel p-3 font-mono text-[13px]" {...props} />,
